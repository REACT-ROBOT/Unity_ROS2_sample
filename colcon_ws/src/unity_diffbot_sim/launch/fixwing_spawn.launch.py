...
def generate_launch_description():
    diffbot_description_path = os.path.join(
        get_package_share_directory('fixedwing_description'))

    xacro_file = os.path.join(diffbot_description_path,
                              'robots',
                              'fixedwing.urdf.xacro')
    urdf_path = os.path.join(diffbot_description_path, 'robots', 'fixedwing.urdf')
    # xacroをロード
    ...
    robot_controllers = PathJoinSubstitution(
        [
            FindPackageShare("fixedwing_description"),
            "config",
            "unity_fixedwing.yaml",
        ]
    )
